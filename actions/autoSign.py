...
class AutoSign:
    ...
    def getUnSignTask(self):
        ...
        if len(res['datas']['unSignedTasks']) < 1:
            raise Exception('所有任务都完成了，当前暂时没有未签到的任务哦！')
        # 获取最后的一个任务
    ...
    def fillForm(self):
        ...
        self.form['isMalposition'] = self.userInfo['isMalposition']
    ...
    def submitForm(self):
        ...
        headers = {
            'tenantId': 'henu',
            'User-Agent': self.session.headers['User-Agent'] + ' okhttp/3.12.4',
            'CpdailyStandAlone': '0',
            'extension': '1',
